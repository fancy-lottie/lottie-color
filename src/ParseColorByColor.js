...
exports.colorify = function (destColors, lottie) {
  ...
  var newLottie = modifyColors(
    modifiedColors,
    lodash_clonedeep_1.default(lottie)
  );
  return newLottie;
};
var convertColorToLottieColor = function (color) {
  if (
    ...
    return [
      (parseInt(result[1], 16) / 255).toFixed(12) - 0,
      (parseInt(result[2], 16) / 255).toFixed(12) - 0,
      (parseInt(result[3], 16) / 255).toFixed(12) - 0,
      1,
    ];
  } else if (
    typeof color === "object" &&
    color.length === 3 &&
    color.every(function (item) {
...
exports.replaceColor = function (sourceColor, targetColor, lottieObj) {
  ...
  function doReplace(sourceLottieColor, targetLottieColor, obj) {
    ...
    if (obj && obj.c && obj.c.k) {
      if (!isNaN(obj.c.k[0])) {
        if (
          sourceLottieColor[0].toFixed(2) === obj.c.k[0].toFixed(2) &&
          sourceLottieColor[1].toFixed(2) === obj.c.k[1].toFixed(2) &&
          sourceLottieColor[2].toFixed(2) === obj.c.k[2].toFixed(2)
        ) {
